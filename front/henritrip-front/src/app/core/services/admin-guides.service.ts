...
export class AdminGuidesService {
  ...
  updateGuideDay(guideId: number, dayId: number, payload: UpdateGuideDayRequest): Observable<{ message: string }> {
    return this.http.put<{ message: string }>(`${this.apiUrl}/guides/${guideId}/days/${dayId}`, payload)
  }

  createActivity(guideId: number, dayId: number, payload: CreateActivityRequest): Observable<{ id: number }> {
    return this.http.post<{ id: number }>(`${this.apiUrl}/guides/${guideId}/days/${dayId}/activities`, payload)
  }

  updateActivity(guideId: number, dayId: number, activityId: number, payload: UpdateActivityRequest): Observable<{ message: string }> {
    return this.http.put<{ message: string }>(`${this.apiUrl}/guides/${guideId}/days/${dayId}/activities/${activityId}`, payload)
  }

  deleteActivity(guideId: number, dayId: number, activityId: number): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/guides/${guideId}/days/${dayId}/activities/${activityId}`)
  }
}
